...
## Part 11: Two Independent New Validations — The Natural Experiment and the Future Season

Parts 6–10 were, one after another, honest nulls: the coaching signal is real (Parts 1–5) but every attempt to find *more* than the single quality number, or to beat the market with it, came back empty. Part 11 turns back to the core signal itself and asks the two sharpest "is it actually useful" questions that had not yet been run — and, unusually for the back half of this project, both come back **positive**.

### 11a. The manager-change event study (a within-club natural experiment)

Every previous coach validation compared coaches *across* clubs, where squad quality is the dominant confounder. This one holds the club fixed. For every one of **2,899 manager changes across the 14 leagues, 2005–2024** — mid-season and between adjacent seasons — it asks whether the model's grade, known *before* the change, predicts what happens *after*, using a first-difference design that sweeps out the club's squad-value level entirely. Leakage is controlled by taking each coach's grade **as-of the outgoing coach's season**, an expanding-window refit that contains neither the before- nor the after-period result.

The naive question — does the *grade gap* between the incoming and outgoing coach predict the change in performance? — is a **null, and instructively so.** It is confounded by selection interacting with regression to the mean: clubs fire a well-graded coach precisely during an unlucky dip, which then reverts regardless of who arrives, forcing a spurious *negative* association. Regression to the mean is the overwhelming force in any manager change (the outgoing performance level carries a coefficient of −0.95). The outgoing coach's quality is simply irrelevant once he is gone.

The right question — does the **absolute grade of who you hire** predict how he does, given where the club was? — is a **clean positive.** Controlling for the club's recent form, the incoming coach's grade predicts his performance-above-squad-value at the new club: **+1.10 (t = 2.90, p = 0.004)**, holding at **+0.91 (p = 0.006)** in a games-weighted mixed model with a club random effect. The effect is **strongest for mid-season crisis hires** (p = 0.003) and weak for considered summer moves (p = 0.25) — the grade matters most exactly where clubs are choosing under the most pressure. This is a fully independent corroboration of the recommender's payoff test (Part 7, p = 0.016), on a different design that no shared squad confounder can explain. **The practical lesson for a sporting director is precise: hiring is about the absolute quality of who you bring in, not how he compares to the man you are replacing.**

**Sacking efficiency.** Because clubs sack on *results* while the model sees *results minus squad-value expectation*, the same machinery grades the sackings themselves. Of 1,667 mid-season sackings, **16.4% fired a coach who was actually overperforming his squad** — bad results, but a worse squad than the club believed. And firing that overperformer **backfires**: the replacement's performance-vs-expectation falls (mean −0.16 PPG; the new man improves on the sacked one only 36% of the time), whereas replacing a genuine underperformer is followed by the expected regression-to-mean bounce (+0.35 PPG, 79%). The model's harshest-sacking list reads like the game's own catalogue of regret — **Birmingham firing an overperforming John Eustace (+0.47) for Wayne Rooney (−0.50) on the way to relegation; Birmingham firing Gary Rowett for Gianfranco Zola** — surfaced purely from the residual, with no knowledge of the narratives.

### 11b. The 2025/26 forward test (a true future holdout)

Every earlier test held out seasons *inside* the 2005–2024 window and cross-validated. This one is stricter: the model was **frozen at 2024** — both the M3 value coefficients and the M5 coach BLUPs estimated only on completed seasons through 2024 — and then used to predict the **2025/26 season it had never seen**, scraped fresh (252 team-seasons, all 14 leagues, game counts matching prior seasons exactly).

**The model generalizes out-of-time.** On the holdout year the minutes-weighted model posts **R² = 0.72, correlation 0.85, and a mean error of 6.3 points per team-season**, and it **still beats raw squad value by 0.013 PPG** — the identical margin the original cross-validation found, now on a genuinely future season, with no degradation. (This is the project's realized-minutes model applied out-of-time, a test of the model's *structure*; the separate pre-season-forecast question is the market benchmark of Part 10.)

**And the coach grades carry genuine forward-looking information.** Coaches the model rated highly on data through 2024 **overperformed in 2025/26 as predicted**: at the stint level, the prior grade predicts the new season's overperformance with a slope of **+1.90 (t = 3.05, p = 0.0024)** — a true out-of-sample future test with zero leakage — and adding the grade to the forecast lowers holdout error (RMSE 0.223 → 0.220). This is the cleanest single confirmation of the coaching signal in the entire project. The season's own story falls straight out: Bayern under Kompany (+18 points over expectation) and Arsenal top the over-achievers, Wolves (−22, relegated) and a faltering Tottenham and Chelsea the under-achievers.

**Taken together, Parts 7, 11a and 11b are three concordant validations of the coach quality grade on three different designs** — new coach-club pairings, a within-club natural experiment, and a true future season — and the market benchmark (Part 10) shows the market pricing the same signal. After a long run of "real but not more than the number, and already priced," this is the project on its firmest ground: the number is real, portable, and it predicts the future.

---

## Part 12: Fan-Facing Surfaces — the Deserved Table and the Development Leaderboard

Two of the project's by-products are, with no new modelling, directly useful to supporters and scouts, and ship as site surfaces.

**The deserved table** restates the M4/M5 residual as a league table: each team's expected points from the model beside where it actually finished, so over- and under-achievement reads as a position swing. It is the core finding made visceral — Leicester 2015/16 finished first but *deserved* tenth (+30 points of overachievement); Chelsea that year deserved first and finished tenth. Its extremes are the residual's extremes (Liverpool 2019 +32), and it carries the same early-small-league caveat as the underlying residual (Limitation 3).

**The player-development leaderboard** is the one durable output of the Part-9 null: the clean player-development residual — market-value growth above a player's own age, price, position and momentum baseline — ranked as a scouting list. Its top names are the canonical breakouts (Ederson €1.2m→€22m, Frenkie de Jong €7m→€85m, Vardy, Emiliano Martínez, Reijnders), and a recent-seasons cut surfaces the selling-league pipelines the CDE analysis identified. It describes *players*, never coaches — Part 9 established that the coach's share of value growth does not repeat — and is labelled as such.
...
13. **Event study (Part 11a):** the leakage-clean design requires each coach's as-of grade, so the fully-graded-on-both-sides subset is only 610 of 2,899 changes; the primary spec treats an unproven incoming coach as the model's prior (BLUP 0), which is the honest forecast but compresses the predictor's range. The positive result is the *level* spec (`resid_in ~ blup_in`); the *difference* spec is a confounded null and is reported only to explain why. Sacking efficiency labels a sacking "harsh" purely on the sign of the outgoing residual, which conflates genuine coaching overperformance with good luck — the backfire result is a regression-to-the-mean statement about the *decision*, not proof the sacked coach was good. Effect sizes are modest (≈ 0.7 pts/season per SD of grade) because one partial-season outcome is very noisy.
14. **Forward test (Part 11b):** Q1 uses the project's realized-minutes weighted value, so it validates the model *structure* out-of-time, not a pre-season forecast (that is Part 10). The 2025/26 holdout is a single season — a strong but n-of-1 future test — and its RMSE beating the 20-year pooled in-sample partly reflects that 2025 avoids the sparse early-2000s league-seasons that inflate the pool. Only 50% of 2025 stints have a prior track-record BLUP; the p = 0.0024 result holds on the graded-only subset (p = 0.0036). The 2025 data is one fresh scrape and inherits the usual Transfermarkt caveats.
15. **Fan surfaces (Part 12):** the deserved table and development leaderboard are descriptive restatements of existing residuals with no new modelling; both inherit their layer's caveats — the deserved table the early-small-league sparsity of Limitation 3, the development leaderboard the Part-9 verdict that value growth is a player-and-market story, not a coach-attributable one (so no coach ranking is drawn from it).
...
Part 11 then turns back to the core signal and, for once in the project's second half, gets two positive answers. A within-club natural experiment over 2,899 manager changes shows that the grade of the coach a club hires predicts his performance-above-squad-value at the new club (p = 0.004), holding the club fixed and strongest exactly where clubs choose under most pressure — the mid-season crisis hire — while a leakage-free forward test on the freshly-scraped 2025/26 season shows the frozen-at-2024 model generalizing out-of-time (R² = 0.72, still beating raw value by the same margin) and its coach grades predicting the *future* season's overperformance (p = 0.0024). With the recommender's payoff test (Part 7), these are three concordant validations of the quality grade on three independent designs — new pairings, a natural experiment, and a genuine future year. The same machinery grades the sackings that produce these changes: one in six mid-season sackings fires a coach who was actually overperforming his squad, and doing so backfires — the model surfaces the game's own catalogue of regret (Eustace-for-Rooney, Rowett-for-Zola) from the residual alone. Part 12 ships two of the project's by-products to supporters as site surfaces — a "deserved table" that renders the residual as league standings, and a development leaderboard built on the one clean, coach-independent output of the Part-9 null.

The project's honest final position is narrow and well-defended: **coaching quality is real, portable, measurable, useful for forecasting a hire against a squad-value baseline, validated on a within-club natural experiment and a true future season, and already priced by the market; it is a single number; and this data cannot yet say what it is made of.** Knowing precisely where the evidence stops — and building the rails that told us — is the result, not a shortfall of one.
